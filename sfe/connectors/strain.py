...
Handles multi-device strain rosette CSV files -> SFEResult.
Parses device/channel naming, groups pairs into within-device
and cross-device, computes diurnal breakdown.

Data format expected:
    Header block ending with DATA_START
    Columns: Time, {device_id}:ch1, {device_id}:ch2, {device_id}:ch3, ...
    Channel axes: ch1=0deg, ch2=45deg, ch3=90deg
    Sample rate: typically 1Hz

Usage
-----
    from sfe.connectors.strain import from_strain_csv, diurnal_breakdown

    result = from_strain_csv("sampledata.csv", W=60)
    groups = result.pair_groups          # within / cross device dicts
    diurnal = diurnal_breakdown(result)  # hour-by-hour rho* and drho
"""
...
def from_strain_csv(path, W=60, delimiter=","):
    ...
    if data.shape[1] < 2:
        raise ValueError(f"Need >= 2 numeric columns, got {data.shape[1]}.")
...
def _print_strain_summary(result):
    ...
    evn = ev / ev.sum()
    bg  = ev[0] / ev[1] if len(ev) > 1 and ev[1] > 1e-12 else float("nan")

    print(f"  -- EIGENSPECTRUM --")
    print(f"  Band gap L1/L2 : {bg:.3f}x")
    print(f"  r_eff joint    : {float(np.nanmean(result.reff_joint)):.4f}")
    print(f"  " + "  ".join(f"L{k+1}={v*100:.1f}%" for k,v in enumerate(evn)))
    print()
...
def diurnal_breakdown(result, pair_idx=0):
    ...
    if len(ts) != len(rho):
        raise ValueError(f"Timestamp length ({len(ts)}) != rho length ({len(rho)}).")
    ...
    for h in range(24):
        mask = hours == h
        if mask.sum() == 0:
            breakdown.append({"hour": h, "rho_star": float("nan"),
...
def strain_figures(result, title_prefix=""):
    ...
    ax.set_title(f"{pfx}rho* Heatmap (dashed = within-device)",
    ...
    try:
        ...
        fig_d, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 6),
        ...
        ax2.set_xlabel("Hour (UTC)", color="#aaa")
